    ...
    <div className="relative mx-auto flex min-h-screen max-w-md flex-col bg-white dark:bg-[var(--color-background-dark)]">
      <div className="sticky top-0 z-10 glass-light px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Chuyển</h1>
        </div>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Chuyển tài sản tới địa chỉ ví.
        </p>
      </div>

      <div className="flex-1 px-6 py-4">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Tính năng đang được phát triển. Bạn có thể rút tiền từ trang Ví.
        </p>
      </div>
    </div>
  );
